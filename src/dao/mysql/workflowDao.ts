...
export class WorkflowRepository implements MYSQLWorkflowRepository {
  ...
  async findWorkflowByName(workflowName: string): Promise<boolean> {
    logger.info(
      "WorkflowRepository --> findWorkflowByName --> workflowName",
      workflowName
    );
  ...
  async getAllWorkflows(): Promise<any> {
    ...
    const [rows] = await dbConnection.execute<RowDataPacket[]>(
      "SELECT * FROM workflow ORDER BY createdAt desc",
      []
    );
    return rows;
  }
  ...
  async findWorkflowDuplicateName(
    workflowId: string,
    workflowName: string
  ): Promise<boolean> {
    logger.info(
  ...
  async updateWorkflowById(
    ...
    await dbConnection.execute<RowDataPacket[]>(
      `UPDATE workflow SET ${setClause} WHERE id = ?`,
      values
    );
  }
  ...
  async activateWorkflow(workflowId: string, status: boolean): Promise<any> {
    logger.info(
      "WorkflowRepository --> activateWorkflow --> workflowId",
      workflowId
    );
    AuditLogger.logAction("activateWorkflow", { workflowId });
    await dbConnection.execute<ResultSetHeader[]>(
      `UPDATE workflow SET isActive = ? WHERE id = ?`,
      [status, workflowId]
    );
  }
  ...
  async findStageByName(stageName: string): Promise<boolean> {
    logger.info(
      "WorkflowRepository --> findStageByName --> stageName",
      stageName
    );
  ...
  async getStageById(stageId: string): Promise<any> {
    ...
    const [rows] = await dbConnection.execute<any[]>(
  ...
  async findStageDuplicatName(
    stageId: string,
    stageName: any
  ): Promise<boolean> {
    logger.info(
      "WorkflowRepository --> findStageDuplicatName --> stageData",
      stageName
    );
  ...
  async activateStage(stageId: string, status: boolean): Promise<any> {
    ...
    await dbConnection.execute<ResultSetHeader[]>(
      `UPDATE stages SET isActive = ? WHERE id = ?`,
      [status, stageId]
    );
  }
  ...
  async addWorkflowInstance(data: any, userDetails: any): Promise<any> {
    ...
    const params = [
      ...
      JSON.stringify(data.currentAllowedUsers),
      data.assetId,
      data.assetType,
      data.requestedBy,
      userDetails.userId,
  ...
  async updateSuggestedAssetData(
    type: string,
    id: string,
    updatedData: any
  ): Promise<void> {
    logger.info(
      "WorkflowRepository --> updateSuggestedAssetData --> updatedData",
      updatedData
    );
    ...
    await dbConnection.execute<RowDataPacket[]>(
      `UPDATE ${type} SET workflowRequests  = ? WHERE id = ?`,
      values
    );
  }
  ...
  async getWorkflowInstanceById(workflowInstanceId: string): Promise<any> {
    logger.info(
      "WorkflowRepository --> getWorkflowInstanceById --> workflowInstanceId",
      workflowInstanceId
    );
    const [rows] = await dbConnection.execute<RowDataPacket[]>(
  ...
  async updateUserLeaveBalance(userId: string, newBalance: any): Promise<any> {
    logger.info(
      "WorkflowRepository --> updateUserLeaveBalance --> userId",
      userId
    );
    AuditLogger.logAction("updateUserLeaveBalance", { userId });
    const [result] = await dbConnection.execute(
      `UPDATE users SET leaveBalance = ? WHERE id = ?`,
      [newBalance, userId]
    );
  ...
  async updateWorkflowInstanceById(id: string, updatedData: any): Promise<any> {
    ...
    let setClause = Object.keys(updatedData[0])
      .map((key) => `${key} = ?`)
      .join(", ");
    const values = Object.values(updatedData[0]);
    values.push(id);
    const [rows] = await dbConnection.execute<RowDataPacket[]>(
      `UPDATE workflow_instances SET ${setClause} WHERE id = ?`,
      values
    );
  }
